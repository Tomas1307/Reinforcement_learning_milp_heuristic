...
REPRESENTATIVE_FILES_DIR = "cluster_results/representative_files"
...
def load_reference_instances():
    ...
    if not ref_files:
        ...
        from heuristic import load_data
...
def assign_to_cluster(instance_features, reference_features, reference_names, scaler):
    """
    Asigna una instancia al cluster más cercano.
    ...
    Returns:
        str: Nombre del cluster asignado
    """
...
def visualize_clustering(instance_features, instance_name, reference_features, reference_names, scaler, assigned_cluster):
    """
    Visualiza la asignación de la instancia a un cluster usando PCA para reducción de dimensionalidad.
    ...
    Args:
        ...
        reference_names: Nombres de las instancias de referencia
        scaler: Objeto StandardScaler para normalizar las características
        assigned_cluster: Nombre del cluster asignado
    """
    ...
    # Línea que conecta la instancia con el cluster asignado
    ...
    plt.title(f'Asignación de {instance_name} al cluster {assigned_cluster}')
    ...
    # Guardar gráfico
    plot_dir = "hyperparameter_analysis/cluster_plots"
...
def get_best_hyperparameters_for_instance(config, instance_name="new_instance", visualize=True):
    ...
    # Asignar instancia al cluster más cercano
    ...
    print(f"Instancia asignada al cluster: {assigned_cluster}")
    ...
    # Devolver los hiperparámetros del cluster asignado
...
def modify_heuristic_with_best_params(heuristica, config, instance_name="new_instance"):
    ...
    Returns:
        tuple: (heuristica modificada, hiperparámetros seleccionados, cluster asignado, features)
    """
    ...
    # Imprimir a qué cluster se asignó la instancia
    print(f"La instancia '{instance_name}' ha sido asignada al cluster: {assigned_cluster}")
    
    # Agregar un print para mostrar los hiperparámetros seleccionados
    print(f"[modify_heuristic_with_best_params] Hiperparámetros seleccionados para {instance_name} (cluster {assigned_cluster}):")
    for param, value in selected_params.items():
        print(f"  {param}: {value}")
    ...
    def run_with_best_params(track_progress=False):
        ...
        print(f"Ejecutando HeurísticaConstructivaEVs con parámetros óptimos para cluster {assigned_cluster}...")
