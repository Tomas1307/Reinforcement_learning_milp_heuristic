...
def cluster_instances_function(features_df, k=None, features_to_use=None):
    ...
    Returns:
        DataFrame: Datos originales con etiquetas de cluster y distancias al centroide
    """
...
def select_representative_instances(clustered_df, n_per_cluster=1):
    """
    Selecciona instancias representativas de cada cluster.
    
    Args:
        clustered_df: DataFrame con instancias agrupadas
        n_per_cluster: Número de instancias a seleccionar por cluster
    ...
    for cluster in clustered_df['cluster'].unique():
        cluster_instances = clustered_df[clustered_df['cluster'] == cluster]
    ...
    for cluster in representative_df['cluster'].unique():
        instances = representative_df[representative_df['cluster'] == cluster]
...
def visualize_clusters(clustered_df, features_to_plot=None):
    ...
    features = [col for col in clustered_df.columns 
               if col not in ['instance_name', 'cluster', 'distance_to_centroid']
    ...
    if len(features_to_plot) > 2:
        ...
        scatter_matrix = pd.plotting.scatter_matrix(
            clustered_df[features_to_plot + ['cluster']], 
...
def analyze_feature_importance(clustered_df):
    ...
    for feature in features:
        cluster_means = clustered_df.groupby('cluster')[feature].mean()
...
def main():
    ...
    parser.add_argument('--n_per_cluster', type=int, default=1,
                        help='Número de instancias representativas por cluster')
    ...
    with open(representatives_path, 'w') as f:
        ...
        for cluster in sorted(representatives_df['cluster'].unique()):
            f.write(f"Cluster {cluster}:\n")
            rep_cluster_data = representatives_df[representatives_df['cluster'] == cluster]
    ...
    with open(report_path, 'w') as f:
        ...
        f.write(f"- Número de clusters: {len(clustered_df['cluster'].unique())}\n")
        f.write(f"- Instancias representativas por cluster: {args.n_per_cluster}\n\n")
        ...
        for cluster in representatives_df['cluster'].unique():
            f.write(f"### Cluster {cluster}\n\n")
            cluster_instances = representatives_df[representatives_df['cluster'] == cluster]
